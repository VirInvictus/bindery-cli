...
- `src/bindery/library.py`: Calibre walk, atomic replace, backups, and native Calibre replacement. Since v0.19.0 `CalibreIdResolver` resolves the `calibredb` book id from `metadata.db` through cquarry (one lazy path→id map per run; since v0.20.0 the map comes from `CalibreDB.format_path_index()`, re-normalized with `resolve().lower()` for the resolver's symlinked-directory and case-insensitive matching) — the `(id)` directory-name regex is only the no-catalog fallback. Never reintroduce directory-name guessing as the primary source: renamed/mismatched directories would replace the wrong book.
